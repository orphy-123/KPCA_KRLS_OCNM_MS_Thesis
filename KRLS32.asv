...
[T f] = size(X);
sumdec=1;
 for nu1 = 0.05:0.05:0.25
     for nu2=0.60:0.05:0.80
        sumdec=sumdec+1;
...
% for b=1:length(jpgFiles)
%     if deltaStore_out(b)>0.5
%         figure(b)
...
Red1
Red2
zFalse = input('Enter # of False: ');
perzFalse(sumdec)=(zFalse/17)*100;
detected(sumdec)=size(Red1,2)+size(Red2,2);
perD=(detected/17)*100;
